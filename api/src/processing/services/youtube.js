...
const normalizeSearchResult = (item) => {
    if (!item || !item.video_id && !item.videoId) {
        return;
    }
    ...
    const description = item.description_snippet?.toString?.?.() ?? item.description?.toString?.?.();
    const published = item.published?.toString?.?.();
    const viewCount = item.view_count?.toString?.?.() ?? item.short_view_count?.toString?.?.();
    const duration = item.length_text?.toString?.?.() ?? item.duration?.text;
    const thumbnail = item.best_thumbnail?.url || item.thumbnails?.[0]?.url;
...
const filterSearchItem = (item) => {
    ...
    if (searchItemTypes.has(item.type)) return true;
    return Boolean(item.video_id || item.videoId);
};

const createSearchClient = async (customFetch) => {
    try {
        return await cloneInnertube(customFetch, Boolean(env.ytSessionServer));
    } catch (e) {
        if (e === "no_session_tokens") {
            return await cloneInnertube(customFetch, false);
        }
        throw e;
    }
};

export async function searchVideos({ query, limit = 20, innertubeClient = env.customInnertubeClient || "WEB_EMBEDDED", dispatcher } = {}) {
    const searchLimit = Math.min(Math.max(Number(limit) || 20, 1), 50);
    const yt = await createSearchClient((input, init) => {
    ...
    const search = await yt.search(query, { client: innertubeClient });
    ...
    return {
        query,
        estimatedResults: Number(search.estimated_results || 0),
        refinements: search.refinement_filters ?? [],
        results,
    };
};
